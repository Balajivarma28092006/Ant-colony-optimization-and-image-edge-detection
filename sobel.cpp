 ...
 *
 * Variant: Sobel-only heuristic (ALPHA = 0).
 *   The ant movement probability is driven exclusively by the Sobel
 *   gradient (eta), with pheromone entirely excluded from the decision
 *   rule (alpha = 0  →  tau^alpha = 1 for every candidate cell).
 *   Pheromone is still deposited and evaporated normally, so the
 *   accumulated pheromone map is what gets visualized — but the ants
 *   themselves navigate by edge strength alone.
 *
 *   Benefits:
 *     • No early stagnation: ants cannot be trapped by an initial
 *       pheromone spike before the map converges.
 *     • Cleaner edge localization: the gradient is the sole guide,
 *       which is more visually intuitive and directly tied to the
 *       Sobel response.
 *
 * Usage:
 *   ./aco_edge_sobel <input_image>
 */
...
/* KEY CHANGE: ALPHA = 0  →  pheromone has NO influence on move choice.
   The probability of stepping to neighbour n is now purely eta[n]^BETA. */
static const float ALPHA          = 0.0f;
...
static const float EDGE_THRESHOLD = 0.1f;   /* threshold relative to TAU_MAX */
...
/* build_heuristic: pure Sobel gradient, normalized to [0,1].
   Pheromone is NOT mixed into eta here — ALPHA = 0 handles that
   on the movement-probability side instead. */
...
int main(int argc, char** argv) {
    ...
    Tigr* screen = tigrWindow(w * 2, h, "ACO Edge Detection — Sobel-Only Heuristic", TIGR_FIXED);
...
#pragma omp parallel for schedule(dynamic, 64)
        for (int ant = 0; ant < NUM_ANTS; ant++) {
            ...
            for (int step = 0; step < ANT_STEPS && alive; step++) {
                ...
                for (int d = 0; d < 8; d++) {
                    ...
                    /* Movement probability: tau^ALPHA * eta^BETA
                       With ALPHA = 0, tau^0 = 1, so this reduces to eta^BETA only.
                       The powf call is kept for generality and compiler optimisation. */
        ...
        /* Pheromone evaporation + deposit + Max-Min clamp */
        for (int k = 0; k < N; k++) {
            tau[k] *= (1.f - RHO);
        ...
        /* Display: threshold-based coloring (original style) */
        for (int i = 0; i < h; i++) {
            for (int j = 0; j < w; j++) {
                int   k        = i * w + j;
    ...
    /* Save result */
    std::string out_path = std::string(in_path) + "_sobel_edges.png";
